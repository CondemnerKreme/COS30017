...
class ReservationActivity : AppCompatActivity() {
    ...
    private fun sendReservationToSheet(reservation: Reservation) {
        ...
        val jsonBody = JSONObject().apply {
            put("name", reservation.customerName)
            put("phone", reservation.phone)
            put("date", reservation.date)
            put("time", reservation.time)
            put("table", reservation.tableNumber)
            put("numPeople", reservation.numPeople)
        }
        ...
        val request = JsonObjectRequest(
            ...
            { _ ->
                Toast.makeText(this, "Error sending to spreadsheet", Toast.LENGTH_SHORT).show()
            }
        )
